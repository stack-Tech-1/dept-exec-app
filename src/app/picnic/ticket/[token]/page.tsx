...
import { CheckCircle, XCircle, Clock, MapPin, Calendar, Download } from 'lucide-react'
...
export default function TicketPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const ticketUrl = typeof window !== 'undefined'
    ...
    : `https://www.ipeexecs.page/picnic/ticket/${token}`

  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`${API}/event-tickets/token/${token}`)
            ...
            <motion.div key="notfound" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center space-y-4"
                ...
                <div>
                  ...
                  <p className="text-sm text-white/30 mt-2">This link is invalid. Contact the social director.</p>
                </div>
          ...
          {state === 'ready' && ticket && (
            ...
            >
              ...
              >
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div>
                    ...
                    {ticket.matricNumber && (
                      <p className="text-white/30 text-sm mt-0.5">{ticket.matricNumber}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-1.5 text-sm">
                    <div className="flex items-center gap-2 text-white/50">
                      ...
                      <span>{new Date(ticket.event.date).toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })} · {ticket.event.time}</span>
                    </div>
                    <div className="flex items-center gap-2 text-white/50">
                    ...
                    {ticket.paymentStatus === 'CONFIRMED' ? (
                      <>
                        ...
                        <p className="text-white/25 text-xs text-center">
                          Show this QR code to collect your items
                        </p>
                      </>
                    ) : (
                      <div className="flex flex-col items-center gap-2 py-4">
                        ...
                        <p className="text-white/30 text-xs text-center">
                          Your QR code will appear here once payment is confirmed by the social director.
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <p className="text-white/20 text-xs text-center px-4">
                Save a screenshot of this page. You'll need your QR code on the day to collect items.
              </p>
            </motion.div>
          )}
